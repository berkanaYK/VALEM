...
public sealed class ConnectionSettingsPage : ContentPage
{
    public ConnectionSettingsPage(ApiClient api)
    {
        ...
        custom.Toggled += (_, e) =>
        {
            url.IsVisible = e.Value;
            status.Text = e.Value ? "Özel sunucu seçildi" : "VALE üretim sunucusu seçildi";
        };
        ...
        reset.Clicked += async (_, _) =>
        {
            api.UseProductionServer();
            custom.IsToggled = false;
            status.Text = "Üretim sunucusu seçildi";
            await DisplayAlertAsync("Bağlantı sıfırlandı", "Eski/özel sunucu ayarı temizlendi.", "Tamam");
        };
            ...
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    ...
                    UiKit.Card(new VerticalStackLayout
                    {
                        ...
                        Children =
                        {
                            new HorizontalStackLayout { Spacing = 10, Children = { custom, UiKit.Label("Özel sunucu kullan", 14, true) } },
                            url,
                            status,
                            test,
                            save,
...
public sealed class DashboardPage : ContentPage
{
    ...
    public DashboardPage(ApiClient api, UserDto user)
    {
        ...
        var m1 = UiKit.Metric("AKTİF", "—", "Parkta");
        _active = m1.Value;
        var m2 = UiKit.Metric("İSTENEN", "—", "Hazırlanıyor");
        _waiting = m2.Value;
        var m3 = UiKit.Metric("BUGÜN", "—", "Teslim edilen");
        _delivered = m3.Value;
        var m4 = UiKit.Metric("CİRO", "—", "Bugünkü tahsilat");
        _revenue = m4.Value;
        ...
        metrics.Add(m1.Card, 0, 0);
        metrics.Add(m2.Card, 1, 0);
        metrics.Add(m3.Card, 0, 1);
        metrics.Add(m4.Card, 1, 1);
        ...
        var list = new CollectionView
        {
            ...
            ItemTemplate = TicketTemplates.CardTemplate(),
            HeightRequest = 320,
            EmptyView = UiKit.Label("Aktif araç yok.", 13, false, true)
        };
            ...
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    new VerticalStackLayout
                    {
                        ...
                        Children =
                        {
                            UiKit.Label("Hoş geldin", 12, false, true),
                            UiKit.Label(user.FullName, 28, true),
                            UiKit.Label(user.BranchName ?? "Şube", 13, false, true)
                        }
                    },
                    metrics,
                    add,
                    UiKit.Label("Aktif araçlar", 20, true),
                    list
                }
            }
        };
    ...
    private async Task RefreshAsync()
    {
        try
        {
            ...
            _active.Text = dashboard.ActiveVehicles.ToString(CultureInfo.CurrentCulture);
            _waiting.Text = dashboard.WaitingForDelivery.ToString(CultureInfo.CurrentCulture);
            _delivered.Text = dashboard.DeliveredToday.ToString(CultureInfo.CurrentCulture);
            _revenue.Text = $"{dashboard.RevenueToday:N2} TL";
            _recent.Clear();
            foreach (var ticket in dashboard.RecentTickets.Take(8)) _recent.Add(ticket);
        }
        catch (Exception ex)
        {
...
public sealed class TicketsPage : ContentPage
{
    ...
    public TicketsPage(ApiClient api)
    {
        ...
        var searchButton = UiKit.SecondaryButton("Ara");
        ...
        searchGrid.Add(_search, 0, 0);
        searchGrid.Add(searchButton, 1, 0);
        root.Add(searchGrid, 0, 1);
    ...
    private async void TicketSelected(object? sender, SelectionChangedEventArgs e)
    {
        ...
        {
            actions.Add("Nakit teslim");
            actions.Add("Kart teslim");
            actions.Add("Havale/EFT teslim");
        }
        if (ticket.Status is TicketStatus.Received or TicketStatus.Parked) actions.Add("Kaydı iptal et");

        if (actions.Count == 0)
        {
            await DisplayAlertAsync(ticket.LicensePlate, "Bu kayıt kapanmış.", "Tamam");
            return;
        }

        var choice = await DisplayActionSheetAsync(ticket.LicensePlate, "Kapat", null, actions.ToArray());
        if (string.IsNullOrWhiteSpace(choice) || choice == "Kapat") return;

        try
        {
            _busy = true;
            if (choice == "Park edildi")
            {
                await _api.UpdateStatusAsync(ticket.Id, TicketStatus.Parked);
            }
            else if (choice == "Araç isteniyor")
            {
                await _api.UpdateStatusAsync(ticket.Id, TicketStatus.Requested);
            }
            else if (choice == "Kaydı iptal et")
            {
                await _api.UpdateStatusAsync(ticket.Id, TicketStatus.Cancelled);
            }
            else
            {
                var method = choice.StartsWith("Nakit", StringComparison.Ordinal)
                    ? PaymentMethod.Cash
                    : choice.StartsWith("Kart", StringComparison.Ordinal)
                        ? PaymentMethod.Card
...
public sealed class NewTicketPage : ContentPage
{
    public NewTicketPage(ApiClient api)
    {
        Title = "Yeni Araç";
        UiKit.StylePage(this);
        var plate = UiKit.Entry("Plaka");
        var brand = UiKit.Entry("Marka");
        var model = UiKit.Entry("Model");
        var color = UiKit.Entry("Renk");
        var customer = UiKit.Entry("Müşteri adı");
        var phone = UiKit.Entry("Telefon", Keyboard.Telephone);
        var key = UiKit.Entry("Anahtar etiketi");
        var spot = UiKit.Entry("Park yeri");
        var notes = UiKit.Entry("Not");
        var rate = UiKit.Entry("Saatlik ücret (boş = varsayılan)", Keyboard.Numeric);
        ...
        save.Clicked += async (_, _) =>
        {
            ...
            {
                if (!decimal.TryParse(rate.Text, NumberStyles.Number, CultureInfo.CurrentCulture, out var value))
                {
            ...
            try
            {
                ...
                await api.CreateTicketAsync(new CreateTicketRequest(
                    null,
                    (plate.Text ?? "").Trim().ToUpperInvariant(),
                    N(brand.Text),
                    N(model.Text),
                    N(color.Text),
                    N(customer.Text),
        ...
        Content = new ScrollView
        {
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    UiKit.Label("Araç kabul", 26, true),
                    UiKit.Label("Temel araç ve müşteri bilgilerini girin.", 13, false, true),
                    UiKit.Card(new VerticalStackLayout
                    {
                        Spacing = 10,
...
public sealed class ReportsPage : ContentPage
{
    ...
    private readonly Label _payments;
    ...
    public ReportsPage(ApiClient api)
    {
        _api = api;
        UiKit.StylePage(this);
        _from = new DatePicker { Date = DateTime.Today.AddDays(-6) };
        _to = new DatePicker { Date = DateTime.Today };
        _from.SetDynamicResource(DatePicker.TextColorProperty, "ValeText");
        _to.SetDynamicResource(DatePicker.TextColorProperty, "ValeText");

        var m1 = UiKit.Metric("ARAÇ", "—", "Dönem toplamı");
        _vehicles = m1.Value;
        var m2 = UiKit.Metric("TESLİM", "—", "Tamamlanan");
        _delivered = m2.Value;
        var m3 = UiKit.Metric("CİRO", "—", "Tahsilat");
        _revenue = m3.Value;
        var m4 = UiKit.Metric("ORTALAMA", "—", "Teslim başına");
        _average = m4.Value;
        ...
        grid.Add(m1.Card, 0, 0);
        grid.Add(m2.Card, 1, 0);
        grid.Add(m3.Card, 0, 1);
        grid.Add(m4.Card, 1, 1);
        ...
        var dailyList = new CollectionView
        {
            ...
            ItemTemplate = new DataTemplate(() =>
            {
                ...
                var detail = UiKit.Label("", 12, false, true);
                detail.SetBinding(Label.TextProperty, new Binding(nameof(DailyReportPointDto.Revenue), stringFormat: "Ciro: {0:N2} TL"));
                return UiKit.Card(new VerticalStackLayout { Spacing = 3, Children = { day, detail } }, new Thickness(12), 14);
            })
        };
        ...
        Content = new ScrollView
        {
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    UiKit.Label("Raporlar", 26, true),
                    UiKit.Label("Operasyon ve tahsilat performansını tarih aralığına göre inceleyin.", 13, false, true),
                    UiKit.Card(new HorizontalStackLayout
                    {
                        Spacing = 12,
                        Children =
                        {
                            new VerticalStackLayout { Children = { UiKit.Label("Başlangıç", 11, true, true), _from } },
                            new VerticalStackLayout { Children = { UiKit.Label("Bitiş", 11, true, true), _to } }
                        }
                    }),
                    load,
                    grid,
                    UiKit.Card(_payments),
                    UiKit.Label("Günlük kırılım", 18, true),
                    dailyList
                }
            }
        };
    ...
    {
        base.OnAppearing();
        if (_daily.Count == 0) await LoadAsync();
    }
    ...
    private async Task LoadAsync()
    {
        try
        {
            ...
            _revenue.Text = $"{report.Revenue:N2} TL";
            _average.Text = $"{report.AverageRevenuePerDelivery:N2} TL";
            _payments.Text = string.Join("  •  ", report.Payments.Select(x => $"{PaymentText(x.Method)}: {x.Amount:N2} TL"));
            _daily.Clear();
            foreach (var day in report.Daily) _daily.Add(day);
        }
        catch (Exception ex)
        {
...
public sealed class ProfilePage : ContentPage
{
    ...
    public ProfilePage(ApiClient api, UserDto user)
    {
        ...
        _user = user;
        UiKit.StylePage(this);
        _name = UiKit.Entry("Ad soyad");
        _name.Text = user.FullName;
        _email = UiKit.Label(user.Email, 14);
        _branch = UiKit.Label(user.BranchName ?? "—", 14);
        _roles = UiKit.Label(string.Join(", ", user.Roles), 14);

        var save = UiKit.PrimaryButton("Profili Kaydet");
        save.Clicked += async (_, _) =>
        {
            try
            {
                _user = await _api.UpdateProfileAsync(_name.Text ?? "");
                await DisplayAlertAsync("Kaydedildi", "Profil güncellendi.", "Tamam");
            }
            catch (Exception ex)
            {
        ...
        Content = new ScrollView
        {
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    new Image { Source = "vale_logo.svg", HeightRequest = 84, WidthRequest = 84, HorizontalOptions = LayoutOptions.Center },
                    UiKit.Label("Profil", 26, true),
                    UiKit.Card(new VerticalStackLayout
                    {
                        Spacing = 10,
                        Children =
                        {
                            UiKit.Label("Ad soyad", 11, true, true),
                            _name,
                            UiKit.Label("E-posta", 11, true, true),
                            _email,
                            UiKit.Label("Şube", 11, true, true),
                            _branch,
                            UiKit.Label("Yetkiler", 11, true, true),
                            _roles,
                            save,
                            password,
                            logout
    ...
    protected override async void OnAppearing()
    {
        ...
        try
        {
            _user = await _api.GetMeAsync();
            _name.Text = _user.FullName;
            _email.Text = _user.Email;
            _branch.Text = _user.BranchName ?? "—";
            _roles.Text = string.Join(", ", _user.Roles);
        }
        catch
        {
            // Profile refresh failure is surfaced by explicit profile actions.
        }
    }
}
...
public sealed class ChangePasswordPage : ContentPage
{
    public ChangePasswordPage(ApiClient api)
    {
        ...
        UiKit.StylePage(this);
        var current = UiKit.Entry("Mevcut parola", password: true);
        var next = UiKit.Entry("Yeni parola", password: true);
        var repeat = UiKit.Entry("Yeni parolayı tekrar", password: true);
        var save = UiKit.PrimaryButton("Parolayı Güncelle");

        save.Clicked += async (_, _) =>
        {
            if (next.Text != repeat.Text)
            {
                await DisplayAlertAsync("Parola", "Yeni parolalar eşleşmiyor.", "Tamam");
                return;
            }

            try
            {
                await api.ChangePasswordAsync(current.Text ?? "", next.Text ?? "");
                await DisplayAlertAsync("Başarılı", "Parolanız değiştirildi.", "Tamam");
                await Navigation.PopAsync();
            }
            catch (Exception ex)
            {
...
public sealed class SettingsPage : ContentPage
{
    public SettingsPage(ApiClient api)
    {
        ...
        {
            Title = "Tema",
            ItemsSource = new[] { "Açık", "Koyu" },
            SelectedIndex = ThemeService.CurrentMode == ValeThemeMode.Dark ? 1 : 0
        };
        picker.SetDynamicResource(Picker.TextColorProperty, "ValeText");
        picker.SelectedIndexChanged += (_, _) =>
            ThemeService.Apply(picker.SelectedIndex == 1 ? ValeThemeMode.Dark : ValeThemeMode.Light);

        var connection = UiKit.SecondaryButton("Sunucu ve Bağlantı Ayarları");
        connection.Clicked += async (_, _) => await Navigation.PushAsync(new ConnectionSettingsPage(api));
        var test = UiKit.SecondaryButton("Canlı Sunucuyu Test Et");
        test.Clicked += async (_, _) =>
        {
            try
            {
                ...
                await DisplayAlertAsync("Bağlantı", "Sunucu ve veritabanı hazır.", "Tamam");
            }
            catch (Exception ex)
            {
        ...
        var reset = UiKit.SecondaryButton("Bağlantı Ayarlarını Sıfırla");
        reset.Clicked += async (_, _) =>
        {
            api.UseProductionServer();
            await DisplayAlertAsync("Sıfırlandı", "VALE üretim sunucusu varsayılan yapıldı.", "Tamam");
        };
        ...
        Content = new ScrollView
        {
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    UiKit.Label("Ayarlar", 26, true),
                    UiKit.Card(new VerticalStackLayout
                    {
                        Spacing = 10,
                        Children =
                        {
                            UiKit.Label("Görünüm", 13, true),
                            picker,
                            UiKit.Label("Uygulama varsayılan olarak açık temada başlar.", 11, false, true)
                        }
                    }),
                    UiKit.Card(new VerticalStackLayout
                    {
                    ...
                    UiKit.Card(new VerticalStackLayout
                    {
                        ...
                        Children =
                        {
                            UiKit.Label("Hakkında", 13, true),
                            UiKit.Label(version, 12, false, true),
                            UiKit.Label("Güvenli oturum, ortak veritabanı ve çoklu şube için tasarlanmıştır.", 11, false, true)
                        }
                    })
                }
            }
...
public sealed class AdminPage : ContentPage
{
    ...
    public AdminPage(ApiClient api)
    {
        ...
        _list = new CollectionView
        {
            ItemsSource = _users,
            SelectionMode = SelectionMode.Single,
            EmptyView = UiKit.Label("Kullanıcı bulunamadı.", 13, false, true),
            ItemTemplate = new DataTemplate(() =>
            {
                var name = UiKit.Label("", 15, true);
                name.SetBinding(Label.TextProperty, nameof(AdminUserDto.FullName));
                var email = UiKit.Label("", 12, false, true);
                email.SetBinding(Label.TextProperty, nameof(AdminUserDto.Email));
                var status = UiKit.Label("", 12, true, true);
                status.SetBinding(Label.TextProperty, nameof(AdminUserDto.StatusText));
                return UiKit.Card(new VerticalStackLayout { Spacing = 3, Children = { name, email, status } }, new Thickness(13), 16);
            })
        };
        ...
        var root = (Grid)Content;
        root.Add(UiKit.Label("Kullanıcı yönetimi", 26, true), 0, 0);
        root.Add(UiKit.Label("Yeni kayıtları onaylayın veya kullanıcı erişimini kapatın.", 12, false, true), 0, 1);
        root.Add(_list, 0, 2);
    }
    ...
    {
        if (e.CurrentSelection.FirstOrDefault() is not AdminUserDto user) return;
        _list.SelectedItem = null;
        var action = user.IsActive ? "Erişimi Kapat" : "Hesabı Onayla";
        var yes = await DisplayAlertAsync(user.FullName, $"{action} işlemi uygulansın mı?", action, "Vazgeç");
        if (!yes) return;
...
public static class TicketTemplates
{
    public static DataTemplate CardTemplate() => new(() =>
    {
        ...
        var vehicle = UiKit.Label("", 12, false, true);
        vehicle.SetBinding(Label.TextProperty, nameof(TicketSummaryDto.VehicleDescription));
        var status = UiKit.Label("", 11, true);
        status.SetBinding(Label.TextProperty, new Binding(nameof(TicketSummaryDto.Status), converter: new TicketStatusTextConverter()));
        var amount = UiKit.Label("", 13, true);
        ...
        grid.Add(new VerticalStackLayout { Spacing = 2, Children = { plate, vehicle } }, 0, 0);
        grid.Add(status, 1, 0);
        grid.Add(amount, 1, 1);
...
public sealed class TicketStatusTextConverter : IValueConverter
{
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture) => value is TicketStatus status
        ? status switch
        {
            ...
            TicketStatus.Parked => "Parkta",
            TicketStatus.Requested => "İsteniyor",
